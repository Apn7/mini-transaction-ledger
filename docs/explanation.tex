...
\addlinespace
\texttt{ApiError} &
One error shape for every error the application raises, so a client never has to guess what a
failure looks like. \\
\addlinespace
Flyway migrations &
...
On the frontend there is one page and therefore one component. Three services ---
\texttt{AccountService}, \texttt{PostingService} and \texttt{HealthService} --- own every HTTP
call; the component holds the screen state in Angular signals and exposes one method per endpoint. There is no router, no state
